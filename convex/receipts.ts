...
import { ReceiptType } from "./types/literals";
...
export const insertReceipt = action({
  ...
  handler: async (ctx, args) => {
    ...
    return { receiptId, url, status: "uploaded" };
  },
});

export const internalInsertReceipt = internalMutation({
  args: {
  ...
  handler: async (ctx, args) => {
    const { fileId, url, fileName, type, userId } = args;
...
export const countUserUploadsSince = internalQuery({
  ...
  handler: async (ctx, { userId, since }) => {
    return await ctx.db
      ...
      .filter((q) => q.gte(q.field("uploadedAt"), since))
      .collect()
